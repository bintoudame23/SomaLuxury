...
const Vetements: React.FC = () => {
  const [produitsVetements, setProduitsVetements] = useState<Produit[]>([]);
  const [cart, setCart] = useState<Produit[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [filter, setFilter] = useState<"default" | "priceAsc" | "priceDesc" | "alpha">("default");
  const [subCategory, setSubCategory] = useState<"all" | "ensemble" | "robe">("all");

  // 🔹 Filtrer les produits de la catégorie "vetements"
  useEffect(() => {
    const vetements = produits.filter(produit => produit.category === "vetements");
    setProduitsVetements(vetements);
  }, []);
  ...
  let produitsFiltres = produitsVetements.filter(produit =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      <main className="max-w-7xl mx-auto py-16 px-4 lg:px-6">
        <h1 className="text-4xl font-bold text-center mb-12 text-gray-900">Vêtements</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-64 bg-white rounded-2xl shadow-md p-6 flex-shrink-0">
            ...
            <div className="flex flex-col gap-3 mb-6">
              ...
              <button
                onClick={() => setSubCategory("ensemble")}
                className={`text-left px-4 py-2 rounded-lg transition ${
                  subCategory === "ensemble" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Ensemble
              </button>
              <button
                onClick={() => setSubCategory("robe")}
                className={`text-left px-4 py-2 rounded-lg transition ${
                  subCategory === "robe" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Robe
              </button>
