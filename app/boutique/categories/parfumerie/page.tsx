...
const Parfumerie: React.FC = () => {
  const [produitsParfumerie, setProduitsParfumerie] = useState<Produit[]>([]);
  const [cart, setCart] = useState<Produit[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [filter, setFilter] = useState<"default" | "priceAsc" | "priceDesc" | "alpha">("default");
  const [subCategory, setSubCategory] = useState<"all" | "homme" | "femme" | "bougies">("all");

  // 🔹 Filtrer les produits de la catégorie "parfumerie"
  useEffect(() => {
    const parfums = produits.filter(produit => produit.category === "parfumerie");
    setProduitsParfumerie(parfums);
  }, []);
  ...
  let produitsFiltres = produitsParfumerie.filter(produit =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      <main className="max-w-7xl mx-auto py-16 px-4 lg:px-6">
        <h1 className="text-4xl font-bold text-center mb-12 text-gray-900">Parfumerie</h1>
          ...
          <aside className="w-full lg:w-64 bg-white rounded-2xl shadow-md p-6 flex-shrink-0">
            ...
            <div className="flex flex-col gap-3 mb-6">
              <button
                ...
                className={`text-left px-4 py-2 rounded-lg transition ${
                  subCategory === "all" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Tous
              </button>
              <button
                onClick={() => setSubCategory("homme")}
                className={`text-left px-4 py-2 rounded-lg transition ${
                  subCategory === "homme" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Parfum Homme
              </button>
              <button
                onClick={() => setSubCategory("femme")}
                className={`text-left px-4 py-2 rounded-lg transition ${
                  subCategory === "femme" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Parfum Femme
              </button>
              <button
                onClick={() => setSubCategory("bougies")}
                className={`text-left px-4 py-2 rounded-lg transition ${
                  subCategory === "bougies" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Bougies
              </button>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className={`text-left px-4 py-2 rounded-lg transition ${
                  filter === "default" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Par défaut
              </button>
              <button
                ...
                className={`text-left px-4 py-2 rounded-lg transition ${
                  filter === "alpha" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                A-Z
              </button>
              <button
                ...
                className={`text-left px-4 py-2 rounded-lg transition ${
                  filter === "priceAsc" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Prix croissant
              </button>
              <button
                ...
                className={`text-left px-4 py-2 rounded-lg transition ${
                  filter === "priceDesc" ? "bg-gray-800 text-white font-semibold" : "hover:bg-pink-50"
                }`}
              >
                Prix décroissant
              </button>
              ...
              produitsFiltres.map((produit) => (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col items-center text-center flex-grow">
                    <h3 className="text-lg font-semibold mb-1 text-gray-800">{produit.name}</h3>
                    <p className="text-gray-700 font-medium mb-3">{produit.price.toLocaleString()} FCFA</p>
                    <button
